/**
 * Topic Memo Screen - Aggregated memo from all contents of a topic
 */
...
import { useTopicMemo } from '../../../hooks';
...
export default function TopicMemoScreen() {
  const { name } = useLocalSearchParams<{ name: string }>();
  const router = useRouter();
  const insets = useSafeAreaInsets();
  const decodedName = decodeURIComponent(name || '');
  const { data: memo, isLoading, error, refetch } = useTopicMemo(decodedName);
  ...
  if (error || !memo) {
    return <ErrorState message={`Mémo "${decodedName}" introuvable`} onRetry={refetch} hasHeader />;
  }
    ...
    <>
      ...
      >
        ...
        <Text variant="caption" color="secondary" style={styles.label}>
          MÉMO • {memo.contentCount} contenu{memo.contentCount > 1 ? 's' : ''}
        </Text>
