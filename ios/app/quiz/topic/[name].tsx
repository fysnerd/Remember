/**
 * Topic Quiz Screen - Mixed questions from all contents of a topic
 */
...
import { useTopicQuiz, useSubmitAnswer, useCreateSession, useCompleteSession } from '../../../hooks';
...
export default function TopicQuizScreen() {
  ...
  const { data: quiz, isLoading, error, refetch } = useTopicQuiz(decodedName);
  ...
  useEffect(() => {
    if (quiz && !sessionId && !createSessionMutation.isPending) {
      createSessionMutation.mutate({ topicName: decodedName }, {
        onSuccess: (session) => {
  ...
  if (error || !quiz || !quiz.questions.length) {
    return <ErrorState message={`Aucun quiz pour "${decodedName}"`} onRetry={refetch} />;
  }
  ...
  const handleViewMemo = () => {
    router.replace({ pathname: '/memo/topic/[name]', params: { name: encodeURIComponent(decodedName) } });
  };
